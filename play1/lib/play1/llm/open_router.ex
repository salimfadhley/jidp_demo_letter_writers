defmodule Play1.LLM.OpenRouter do
  @moduledoc """
    ...
    with {:ok, key} <- api_key() do
      ...
      request =
        Req.new(
          ...
          auth: {:bearer, key},
          headers: [
            {"http-referer", "https://github.com/salimfadhley/play1"},
            {"x-title", "play1"}
          ],
  ...
  def default_model do
    Application.get_env(:play1, :openrouter_model, "anthropic/claude-sonnet-5")
  end

  defp api_key do
    case Application.get_env(:play1, :openrouter_api_key) do
